...
async def on_ready():
    await init_db()
    print(f"Albion Bot Ready as {bot.user}")

# PvE LFG
@bot.slash_command(description="Create PvE group")
async def lfg(inter):
    view = LFGView()
    await inter.send(embed=view.build_embed(), view=view)
...
async def craft(
    ...
    embed = disnake.Embed(title="💰 Craft Profit", color=0x00ff9c)
...
async def refine(
    ...
    embed = disnake.Embed(title="⚒ Refine Profit", color=0x00ffff)
    embed.add_field(name="City", value=city)
    embed.add_field(name="Profit", value=f"{profit} silver")
    await inter.send(embed=embed)
